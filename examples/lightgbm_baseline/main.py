...
DEFAULT_PREDICT_THREADS = 4


def _predict_thread_count() -> int:
    raw_value = os.environ.get("LIGHTGBM_BASELINE_PREDICT_THREADS", str(DEFAULT_PREDICT_THREADS))
    try:
        value = int(raw_value)
    except ValueError as exc:
        raise ValueError("LIGHTGBM_BASELINE_PREDICT_THREADS must be an integer") from exc
    if value < 1:
        raise ValueError("LIGHTGBM_BASELINE_PREDICT_THREADS must be at least 1")
    return value
...
class Model:
    ...
    def __init__(self):
        ...
        self.predict_threads = _predict_thread_count()
    ...
    def predict(self, test: pd.DataFrame) -> np.ndarray:
        ...
        preds = [
            booster.predict(
                ...
                num_threads=self.predict_threads,
            )
