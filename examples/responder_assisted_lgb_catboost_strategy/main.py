...
class Model:
    def __init__(self):
        ...
        for name, model in zip(self.responders, self.responder_models):
        ...
        raw_base_feature_count = len(self.features) + 1
        expected_target_features = {
            "A": raw_base_feature_count,
            "B": self.base_feature_count,
            "C": raw_base_feature_count + len(self.responders),
            "D": self.base_feature_count + len(self.responders),
        }[self.target_variant]
    ...
    def predict(self, test):
        ...
        target_x = {
            "A": raw_base,
            "B": base,
            "C": np.hstack([raw_base, responder_hat]),
            "D": np.hstack([base, responder_hat]),
        }[self.target_variant]
